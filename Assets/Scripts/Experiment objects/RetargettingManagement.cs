...
public class RetargettingManagement : MonoBehaviour
{
    ...
    public class AssociatePoint{
        ...
        public Vector3 getVectDiff()
        {
            ...
            foreach (Transform child in display)
            {
                ...
                {
                    return child.position - invisible.position;
                }
            }
            return display.position - invisible.position;
        }
    }
    ...
    private void addKnucles(GameObject hand, string displayFingerName, string invisibleFingerName)
    {
        ...
        foreach (string knuckle in new string[] { "a", "b", "c", "end" })
        {
            ...
            Debug.Log(invisible_name);
            Debug.Log(display_name);

            Debug.Log(findTransform(left_invisible_hand.transform, name));
            Debug.Log(findTransform(hand.transform, name));
    ...
    Vector3 getFingerOffset(List<AssociatePoint> knuckles)
    {
        ...
        {
            offset += ap.getVectDiff() * (invertPower(ap)) / totalInvertedDist;
        }
        //Debug.Log(offset);
        return offset;
    }
